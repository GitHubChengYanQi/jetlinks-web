...
const Association: React.FC<EditableTableProps> = (props) => {
      ...
      render: (text: string[]) => {
        if (text && text.length > 0) {
          return text.map(e => {
            ...
            return <Col span={12} key={e}><Tag color={"green"} style={{
              width: 70,
              textAlign: "center",
              marginBottom: 4
            }}> {(item || {}).describe} </Tag></Col>
          });
        } else {
          return text;
        }
    ...
    },
    {
      ...
      render: (text: string[]) => {
        if (text && text.length > 0) {
          return text.map(e => {
            const item = defaultActionData.find(i => i.action === e);
            return <Col span={12} key={e}><Tag color={"blue"} style={{
              width: 70,
              textAlign: "center",
              marginBottom: 4
            }}> {(item || {}).describe} </Tag></Col>
  ...
  return (
    <EditableContext.Provider value={{...form, permissions}}>
      <Table
        size="small"
