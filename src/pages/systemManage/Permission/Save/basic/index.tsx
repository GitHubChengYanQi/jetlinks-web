...
const Basic: React.FC<Props> = props => {
  ...
  PubSub.subscribe('permission-basic-save', (topic: any, data: any) => {
    form.validateFields().then((fieldValue) => {
      ...
      data.callback({...fieldValue, properties: {type: fieldValue.type}});
    });
  });

  return (
    <div>
      <Form form={form} labelAlign={'right'} className={style['ant-form-item']}>
        <Row>
          <Col span={12}>
            <Form.Item
              key="id"
            ...
            >
              <Input placeholder="只能由字母数字下划线组成" readOnly={!!data.id} />
            </Form.Item>
          </Col>
