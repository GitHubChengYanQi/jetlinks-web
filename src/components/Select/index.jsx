...
const Select = (props) => {
  ...
  if (data) {
    return (
      <>
        {!loading &&<AntSelect bordered={border} options={data} disabled={disabled}  placeholder={placeholder} allowClear style={{ width: wid }} value={valueArray} {...other} filterOption={(input, option) =>option.label.toLowerCase().indexOf(input.toLowerCase()) >= 0}
        />}
      </>
    );
  } else {
